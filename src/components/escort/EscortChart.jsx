...
const EscortChart = ({ startDate, endDate }) => {
  ...
  const data = {
    labels: Object.keys(escortingCounts),
    datasets: [
      {
        label: 'Escorting Counts',
        data: Object.values(escortingCounts).map(option => option.count),
        backgroundColor: [
          'rgba(255, 99, 132, 0.6)',
          'rgba(54, 162, 235, 0.6)',
          'rgba(255, 206, 86, 0.6)',
      ...
      tooltip: {
        callbacks: {
          label: function(context) {
            ...
            const optionData = escortingCounts[option];
            return `${option}: ${optionData.count} records, ${optionData.totalNight} nights`;
          }
        }
      }
    }
